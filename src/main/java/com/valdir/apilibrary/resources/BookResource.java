...
public class BookResource {
	...
	@ApiOperation(value = "update a book by id whith PUT method")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Success method")
	})
	@PutMapping(value = "/books/{id}")
	public ResponseEntity<BookDTO> updatePost(@PathVariable Integer id, @RequestBody BookDTO obj) {
		Book newObj = service.update(id, obj);
		obj = new BookDTO(newObj);
		return ResponseEntity.ok().body(obj);
	}
	
	@ApiOperation(value = "update a book by id whith PATCH method")
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Success method")
	})
	@PatchMapping(value = "/books/{id}")
	public ResponseEntity<BookDTO> updatePatch(@PathVariable Integer id, @RequestBody BookDTO obj) {
		Book newObj = service.update(id, obj);
		obj = new BookDTO(newObj);
		return ResponseEntity.ok().body(obj);
	}
